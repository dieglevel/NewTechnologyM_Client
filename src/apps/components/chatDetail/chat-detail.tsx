...
const ChatDetail = () => {
	...
	const RenderMessageItem = ({ item }: { item: IMessage }) => {
		...
		useEffect(() => {
			const getMyID = async () => {
				const id = await getSecure(ExpoSecureStoreKeys.UserId);
				console.log("ID", id);
	...
	const renderActionItem = ({
		...
		>
			<Ionicons
				...
				size={20}
				color={item.destructive ? "#ef4444" : colorScheme ? "#d1d5db" : "#374151"}
			...
			<Text
				style={[
					...
					colorScheme && !item.destructive && styles.darkActionText,
				]}
			>
				{item.label}
			</Text>
	...
	const RenderChatItem = ({ item }: ChatItem) => (
		...
		>
			...
			/>
			<Text style={[styles.chatName, colorScheme && styles.darkChatName]}>{item.accountId}</Text>
		</TouchableOpacity>
	);
	...
	return (
		<View style={[styles.container, colorScheme && styles.darkContainer]}>
			<View style={styles.header}>
				...
				<Text style={styles.headerText}>{route.params.room.name}</Text>
			...
			{showSearchBar && (
				<View style={[styles.searchContainer, colorScheme && styles.darkSearchContainer]}>
					<TextInput
						style={[styles.searchInput, colorScheme && styles.darkSearchInput]}
						placeholder="Tìm kiếm tin nhắn..."
						placeholderTextColor={colorScheme ? "#9ca3af" : "#6b7280"}
						value={searchQuery}
      ...
      /> */}
			...
			{isTyping && !showSearchBar && (
				<Text style={[styles.typingIndicator, colorScheme && styles.darkTypingIndicator]}>Đang gõ...</Text>
			)}
			...
			>
				...
				>
					<View style={[styles.actionModalContent, colorScheme && styles.darkActionModalContent]}>
						{/* <FlatList
              data={getActionItems()}
			...
			>
				...
				>
					<View style={[styles.forwardModalContent, colorScheme && styles.darkForwardModalContent]}>
						<Text style={[styles.forwardModalTitle, colorScheme && styles.darkForwardModalTitle]}>
			...
			>
				<View style={styles.inputContainer}>
					...
					<TextInput
						style={[styles.textInput, colorScheme && styles.darkTextInput]}
